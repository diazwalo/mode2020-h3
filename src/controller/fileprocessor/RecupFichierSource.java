...
public class RecupFichierSource {
	...
	public int donneeFichier(String cheminFichier){
		...
		try{
			...
			InputStream flux=new FileInputStream(ligne1);
			...
			if(tab[0].equals("PARAMS")) {
				...
				if (tab[1].equals("Fixe")) {
					...
					if(masse == 0 || rayon == 0 || nomf == null){
						System.out.println("Problème de déclaration d'un objet fixe, vérifiez que la masse et le rayon ne sont pas égale à 0");
						return 1;
					}
					...
					if (tab[1].equals("Simulé")) {
						...
						if (tab[1].equals("Ellipse")) {
							System.out.println("Le programme ne prends pas encore en charge les ellipses\n Utilisez un fichier source avec des objets sumulés");
							System.exit(1);
